...
1. 可以有输出语句。
  ...
  5. **每次创建对象的时候，都会执行一次。且先于当前类的构造器执行。**
...
> ### 完整执行顺序
>
> 当一个子类同时包含静态和实例初始化块时，从程序开始到对象创建的完整顺序如下：
>
> #### 第一阶段：类加载（在JVM中仅发生一次）
>
> 当JVM首次需要使用某个类时（比如创建它的实例或调用它的静态方法），会触发类加载。
>
> 1. **加载父类**：如果子类有父类，会先加载父类。
> 2. **执行父类静态初始化**：执行父类的 `static` 初始化块和静态成员变量的赋值，按代码顺序执行。
> 3. **加载子类**：父类加载完毕后，加载子类。
> 4. **执行子类静态初始化**：执行子类的 `static` 初始化块和静态成员变量的赋值，按代码顺序执行。
>
> *这个阶段是“静态”部分，与创建对象无关，只在类第一次被用到时执行。*
>
> #### 第二阶段：对象创建（每次 `new` 都会完整执行一遍）
>
> 1. **执行父类实例初始化**：
>    - 执行父类的实例初始化块和实例成员变量的赋值。
>    - 执行父类的构造器体。
> 2. **执行子类实例初始化**：
>    - 执行子类的实例初始化块和实例成员变量的赋值。
>    - 执行子类的构造器体。
