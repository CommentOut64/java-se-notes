...
* **编译时期异常**(checked 异常、**受检异常**)：在代码编译阶段，编译器就能明确警示当前代码`可能发生`的异常，并`明确督促`程序员提前编写处理它的代码。如果程序员没有编写对应的异常处理代码，则编译器就会`直接判定编译失败`，从而不能生成字节码文件。通常这类异常的发生不是由程序员的代码引起的，或者不是靠加简单判断就可以避免的。
...
public void test1()
{
    ...
    {
        // 异常的处理方式 1
        System.out.println("不好意思，出现了小问题，正在加紧解决...");
...
如果在编写**方法体**的代码时，某句代码可能发生某个`编译时异常`，不处理编译不通过，但是在当前方法体中可能`不适合处理`或`无法给出合理的处理方式`，则此方法应`显式地`声明抛出异常，表明该方法将不对这些异常进行处理，而由该方法的调用者负责处理。
...
3. 返回值类型
   ...
   引用数据类型：子类中重写该方法时，返回值类型可以是**相同类型**或是其**子类型**

4. 权限修饰符：子类重写方法时，访问权限**不能**比父类中被重写的方法更**严格**，只能相同或更**宽松**，而且要求父类被重写方法在子类中是可见的
...
public class TestThrow {
    ...
    public static int max(int... nums){ // 不会强制要求使用 throws
